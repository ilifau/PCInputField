	...
	this.send = function (send_button) {
		...
		$.ajax({
			...
			.done(function (data) {
				...
				$('#status_' + input_name).html(texts.submitted + ' ' + data.submit_time_str);

				//Change send button to re-submit
				$('input#' + input_name + '_' + exercise_id + '_' + assignment_id).attr('value', texts.re_submit);

				// give positive feedback
				alert(texts.submit_success);
			});
	}
};
